"use client"

import { Button } from "@/components/ui/button"
import { Input } from "@/components/ui/input"
import { Card, CardContent } from "@/components/ui/card"
import { Link, ExternalLink, ArrowRight, Copy, Scissors } from "lucide-react"
import { useState, useRef } from "react"

export default function Home() {
  const [inputUrl, setInputUrl] = useState("");
  const [result, setResult] = useState("");
  const inputRef = useRef<HTMLInputElement>(null);

  async function handleShorten(e: React.FormEvent) {
    e.preventDefault();
    setResult("");
    if (!inputUrl) return;
    try {
      const res = await fetch("http://localhost:5000/shorten", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ url: inputUrl })
      });
      const data = await res.json();
      setResult(data.result);
    } catch (err) {
      setResult("Error al acortar la URL");
    }
  }

  return (
    <div className="flex min-h-screen flex-col">
      <header className="border-b">
        <div className="container flex h-16 items-center justify-between px-4 md:px-6">
          <div className="flex items-center gap-2">
            <Link className="h-6 w-6" />
            <span className="text-xl font-bold">Lsai</span>
          </div>
          <nav className="hidden md:flex gap-6">
            <a href="#" className="text-sm font-medium hover:underline">
              Inicio
            </a>
            <a href="#" className="text-sm font-medium hover:underline">
              Características
            </a>
            <a href="#" className="text-sm font-medium hover:underline">
              Precios
            </a>
            <a href="#" className="text-sm font-medium hover:underline">
              Contacto
            </a>
          </nav>
          <div className="flex items-center gap-4">
            <Button variant="outline" size="sm" className="hidden md:flex">
              Iniciar Sesión
            </Button>
            <Button size="sm">Registrarse</Button>
          </div>
        </div>
      </header>
      <main className="flex-1">
        <section className="w-full py-12 md:py-24 lg:py-32 bg-gradient-to-b from-white to-gray-100 dark:from-gray-900 dark:to-gray-800">
          <div className="container px-4 md:px-6">
            <div className="flex flex-col items-center justify-center space-y-4 text-center">
              <div className="space-y-2">
                <h1 className="text-3xl font-bold tracking-tighter sm:text-4xl md:text-5xl lg:text-6xl">
                  Acorta tus URLs con <span className="text-primary">IA</span>
                </h1>
                <p className="mx-auto max-w-[700px] text-gray-500 md:text-xl dark:text-gray-400">
                  Simplifica tus enlaces largos en URLs cortas y fáciles de compartir en segundos.
                </p>
              </div>
              <div className="w-full max-w-md space-y-2">
                <Card className="border-2">
                  <CardContent className="p-3">
                    <form className="flex flex-col space-y-4 sm:flex-row sm:space-y-0 sm:space-x-2" onSubmit={handleShorten}>
                      <div className="relative flex-1">
                        <ExternalLink className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-gray-500" />
                        <Input
                          type="url"
                          placeholder="Pega tu enlace largo aquí"
                          className="pl-10"
                          value={inputUrl}
                          onChange={e => setInputUrl(e.target.value)}
                          ref={inputRef}
                        />
                      </div>
                      <Button type="submit" className="gap-1">
                        <Scissors className="h-4 w-4" />
                        Acortar
                      </Button>
                    </form>
                    {result && (
                      <div className="mt-4 text-sm text-center text-primary font-semibold break-all">
                        Resultado: {result}
                      </div>
                    )}
                  </CardContent>
                </Card>
                <p className="text-xs text-gray-500 dark:text-gray-400">
                  Al usar nuestro servicio, aceptas nuestros Términos y Condiciones.
                </p>
              </div>
            </div>
          </div>
        </section>
        <section className="w-full py-12 md:py-24 lg:py-32">
          <div className="container px-4 md:px-6">
            <div className="mx-auto grid max-w-5xl items-center gap-6 py-12 lg:grid-cols-2 lg:gap-12">
              <div className="space-y-4">
                <div className="inline-block rounded-lg bg-gray-100 px-3 py-1 text-sm dark:bg-gray-800">
                  Cómo funciona
                </div>
                <h2 className="text-3xl font-bold tracking-tighter md:text-4xl">
                  Acorta, comparte y analiza tus enlaces
                </h2>
                <p className="text-gray-500 dark:text-gray-400">
                  Lsai te permite acortar URLs largas en enlaces cortos y fáciles de recordar. Además, obtén
                  estadísticas detalladas sobre quién hace clic en tus enlaces.
                </p>
                <div className="flex flex-col gap-2 min-[400px]:flex-row">
                  <Button className="gap-1">
                    Empezar ahora
                    <ArrowRight className="h-4 w-4" />
                  </Button>
                  <Button variant="outline">Saber más</Button>
                </div>
              </div>
              <div className="grid gap-6 sm:grid-cols-2">
                <Card>
                  <CardContent className="p-6">
                    <div className="flex items-center gap-4">
                      <div className="rounded-full bg-primary/10 p-3">
                        <Scissors className="h-6 w-6 text-primary" />
                      </div>
                      <div>
                        <h3 className="font-bold">Acorta URLs</h3>
                        <p className="text-sm text-gray-500 dark:text-gray-400">
                          Convierte URLs largas en enlaces cortos y fáciles de compartir.
                        </p>
                      </div>
                    </div>
                  </CardContent>
                </Card>
                <Card>
                  <CardContent className="p-6">
                    <div className="flex items-center gap-4">
                      <div className="rounded-full bg-primary/10 p-3">
                        <Copy className="h-6 w-6 text-primary" />
                      </div>
                      <div>
                        <h3 className="font-bold">Copia y comparte</h3>
                        <p className="text-sm text-gray-500 dark:text-gray-400">
                          Copia tus enlaces cortos con un clic y compártelos fácilmente.
                        </p>
                      </div>
                    </div>
                  </CardContent>
                </Card>
                <Card>
                  <CardContent className="p-6">
                    <div className="flex items-center gap-4">
                      <div className="rounded-full bg-primary/10 p-3">
                        <Link className="h-6 w-6 text-primary" />
                      </div>
                      <div>
                        <h3 className="font-bold">Enlaces personalizados</h3>
                        <p className="text-sm text-gray-500 dark:text-gray-400">
                          Crea URLs cortas personalizadas con tu propio texto.
                        </p>
                      </div>
                    </div>
                  </CardContent>
                </Card>
                <Card>
                  <CardContent className="p-6">
                    <div className="flex items-center gap-4">
                      <div className="rounded-full bg-primary/10 p-3">
                        <ExternalLink className="h-6 w-6 text-primary" />
                      </div>
                      <div>
                        <h3 className="font-bold">Estadísticas</h3>
                        <p className="text-sm text-gray-500 dark:text-gray-400">
                          Analiza quién, cuándo y dónde hacen clic en tus enlaces.
                        </p>
                      </div>
                    </div>
                  </CardContent>
                </Card>
              </div>
            </div>
          </div>
        </section>
      </main>
      <footer className="border-t bg-gray-100 dark:bg-gray-800">
        <div className="container flex flex-col gap-4 py-10 md:flex-row md:gap-8 px-4 md:px-6">
          <div className="flex flex-col gap-2 md:gap-4 md:w-1/3">
            <div className="flex items-center gap-2">
              <Link className="h-6 w-6" />
              <span className="text-xl font-bold">Lsai</span>
            </div>
            <p className="text-sm text-gray-500 dark:text-gray-400">
              Simplifica tus enlaces largos en URLs cortas y fáciles de compartir en segundos.
            </p>
          </div>
          <div className="grid flex-1 grid-cols-2 gap-8 sm:grid-cols-3">
            <div className="space-y-3">
              <h3 className="text-sm font-medium">Producto</h3>
              <ul className="space-y-2">
                <li>
                  <a href="#" className="text-sm hover:underline">
                    Características
                  </a>
                </li>
                <li>
                  <a href="#" className="text-sm hover:underline">
                    Precios
                  </a>
                </li>
                <li>
                  <a href="#" className="text-sm hover:underline">
                    API
                  </a>
                </li>
              </ul>
            </div>
            <div className="space-y-3">
              <h3 className="text-sm font-medium">Compañía</h3>
              <ul className="space-y-2">
                <li>
                  <a href="#" className="text-sm hover:underline">
                    Acerca de
                  </a>
                </li>
                <li>
                  <a href="#" className="text-sm hover:underline">
                    Blog
                  </a>
                </li>
                <li>
                  <a href="#" className="text-sm hover:underline">
                    Contacto
                  </a>
                </li>
              </ul>
            </div>
            <div className="space-y-3">
              <h3 className="text-sm font-medium">Legal</h3>
              <ul className="space-y-2">
                <li>
                  <a href="#" className="text-sm hover:underline">
                    Términos
                  </a>
                </li>
                <li>
                  <a href="#" className="text-sm hover:underline">
                    Privacidad
                  </a>
                </li>
                <li>
                  <a href="#" className="text-sm hover:underline">
                    Cookies
                  </a>
                </li>
              </ul>
            </div>
          </div>
        </div>
        <div className="border-t py-6">
          <div className="container flex flex-col items-center justify-between gap-4 md:flex-row px-4 md:px-6">
            <p className="text-xs text-gray-500 dark:text-gray-400">© 2025 Lsai. Todos los derechos reservados.</p>
            <div className="flex gap-4">
              <a href="#" className="text-gray-500 hover:text-gray-900 dark:text-gray-400 dark:hover:text-gray-50">
                <svg className="h-5 w-5" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                  <path
                    fillRule="evenodd"
                    d="M22 12c0-5.523-4.477-10-10-10S2 6.477 2 12c0 4.991 3.657 9.128 8.438 9.878v-6.987h-2.54V12h2.54V9.797c0-2.506 1.492-3.89 3.777-3.89 1.094 0 2.238.195 2.238.195v2.46h-1.26c-1.243 0-1.63.771-1.63 1.562V12h2.773l-.443 2.89h-2.33v6.988C18.343 21.128 22 16.991 22 12z"
                    clipRule="evenodd"
                  />
                </svg>
              </a>
              <a href="#" className="text-gray-500 hover:text-gray-900 dark:text-gray-400 dark:hover:text-gray-50">
                <svg className="h-5 w-5" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                  <path
                    fillRule="evenodd"
                    d="M12.315 2c2.43 0 2.784.013 3.808.06 1.064.049 1.791.218 2.427.465a4.902 4.902 0 011.772 1.153 4.902 4.902 0 011.153 1.772c.247.636.416 1.363.465 2.427.048 1.067.06 1.407.06 4.123v.08c0 2.643-.012 2.987-.06 4.043-.049 1.064-.218 1.791-.465 2.427a4.902 4.902 0 01-1.153 1.772 4.902 4.902 0 01-1.772 1.153c-.636.247-1.363.416-2.427.465-1.067.048-1.407.06-4.123.06h-.08c-2.643 0-2.987-.012-4.043-.06-1.064-.049-1.791-.218-2.427-.465a4.902 4.902 0 01-1.772-1.153 4.902 4.902 0 01-1.153-1.772c-.247-.636-.416-1.363-.465-2.427-.047-1.024-.06-1.379-.06-3.808v-.63c0-2.43.013-2.784.06-3.808.049-1.064.218-1.791.465-2.427a4.902 4.902 0 011.153-1.772A4.902 4.902 0 015.45 2.525c.636-.247 1.363-.416 2.427-.465C8.901 2.013 9.256 2 11.685 2h.63zm-.081 1.802h-.468c-2.456 0-2.784.011-3.807.058-.975.045-1.504.207-1.857.344-.467.182-.8.398-1.15.748-.35.35-.566.683-.748 1.15-.137.353-.3.882-.344 1.857-.047 1.023-.058 1.351-.058 3.807v.468c0 2.456.011 2.784.058 3.807.045.975.207 1.504.344 1.857.182.466.399.8.748 1.15.35.35.683.566 1.15.748.353.137.882.3 1.857.344 1.054.048 1.37.058 4.041.058h.08c2.597 0 2.917-.01 3.96-.058.976-.045 1.505-.207 1.858-.344.466-.182.8-.398 1.15-.748.35-.35.566-.683.748-1.15.137-.353.3-.882.344-1.857.048-1.055.058-1.37.058-4.041v-.08c0-2.597-.01-2.917-.058-3.96-.045-.976-.207-1.505-.344-1.858a3.097 3.097 0 00-.748-1.15 3.098 3.098 0 00-1.15-.748c-.353-.137-.882-.3-1.857-.344-1.023-.047-1.351-.058-3.807-.058zM12 6.865a5.135 5.135 0 110 10.27 5.135 5.135 0 010-10.27zm0 1.802a3.333 3.333 0 100 6.666 3.333 3.333 0 000-6.666zm5.338-3.205a1.2 1.2 0 110 2.4 1.2 1.2 0 010-2.4z"
                    clipRule="evenodd"
                  />
                </svg>
              </a>
              <a href="#" className="text-gray-500 hover:text-gray-900 dark:text-gray-400 dark:hover:text-gray-50">
                <svg className="h-5 w-5" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                  <path d="M8.29 20.251c7.547 0 11.675-6.253 11.675-11.675 0-.178 0-.355-.012-.53A8.348 8.348 0 0022 5.92a8.19 8.19 0 01-2.357.646 4.118 4.118 0 001.804-2.27 8.224 8.224 0 01-2.605.996 4.107 4.107 0 00-6.993 3.743 11.65 11.65 0 01-8.457-4.287 4.106 4.106 0 001.27 5.477A4.072 4.072 0 012.8 9.713v.052a4.105 4.105 0 003.292 4.022 4.095 4.095 0 01-1.853.07 4.108 4.108 0 003.834 2.85A8.233 8.233 0 012 18.407a11.616 11.616 0 006.29 1.84" />
                </svg>
              </a>
            </div>
          </div>
        </div>
      </footer>
    </div>
  )
}
